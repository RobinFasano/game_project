...
function draw()
{
	...
	if (flagpole.isReached) {
		fill(255);
		noStroke();
		text("Level complete. Press space to continue.", 200, 200);
		return;
	}
...
function keyPressed()
{
	if (keyCode == 37 && flagpole.isReached == false) //left arrow
	{
		isLeft = true;
	}
	else if (keyCode == 39 && flagpole.isReached == false) { //right arrow
		isRight = true;
	}
	else if (keyCode == 32 && flagpole.isReached == false) { //space bar
		//make sure the character is on the ground
		if (gameChar_y == floorPos_y) {
			gameChar_y -= 100;
		}
	}
	else if (keyCode == 32 && (flagpole.isReached == true || lives < 1)) { //space bar
		setup();
	}
}
...
function renderFlagpole()
{
	...
	if (flagpole.isReached)
	{
		rect(flagpole.x_pos + 2.5, floorPos_y - 250, 50, 50);
	}
	else
	{
		rect(flagpole.x_pos + 2.5, floorPos_y - 50, 50, 50);
	}
	pop();
}
...
{
	if (dist(gameChar_world_x, gameChar_y, t_collectable.x_pos, t_collectable.y_pos) < 41)
	{
...
function startGame()
{
	...
	collectables = [
		{x_pos: -650, y_pos: 410, isFound: false},
		{x_pos: -450, y_pos: 410, isFound: false},
		{x_pos: 30, y_pos: 410, isFound: false},
		{x_pos: 300, y_pos: 410, isFound: false},
		{x_pos: 700, y_pos: 410, isFound: false},
		{x_pos: 1400, y_pos: 410, isFound: false}
	];
