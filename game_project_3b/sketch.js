...
function setup()
{
	...
	collectable = {x_pos: 200, y_pos: 400, width: 40, height: 40, isFound: false};
...
function draw()
{
	...
	//if collectable is not found yet, draw collectable
	if (collectable.isFound == false) {
		//draw collectable
		fill(255,255,0);
		ellipse(collectable.x_pos, collectable.y_pos, collectable.width, collectable.height);
		fill(255, 193, 037);
		ellipse(collectable.x_pos, collectable.y_pos, collectable.width - 10, collectable.height - 10);
		fill(0);
		textSize(20);
	...
	if (gameChar_x > canyon.x_pos + 15 && gameChar_x < canyon.x_pos + (canyon.width - 15)) {
		isPlummeting = true;
	}
	else {
		isPlummeting = false;
	}
	//if character is over canyon, character will fall
